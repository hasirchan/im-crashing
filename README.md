...
- If `im-crashing` does not detect the `RIME_SHARED_DATA_DIR` environment variable at startup, it will crash immediately because it expects to locate the default configuration file in `RIME_SHARED_DATA_DIR`.
